    ...
    public class DiscordRichPresenceLogic
    {
            ...
            await updateExecutor.ExecuteAsync(async () =>
            {
                ...
                else
                {
                    try
                    {
                        ...
                        {
                            tooltip += " 🧑‍✈️✈️ Flying Near " + airport;
                        }
                        if(!string.IsNullOrEmpty(icao))
                        {
                            details += $" 🧑‍✈️✈ Flying Near {icao}";
                            tooltip += $" ({icao})";
                        }
                        if(!string.IsNullOrEmpty(country))
                        {
                            details += $", 🧑‍✈️✈ Flying Near {country}";
                            tooltip += $" {country}";
                        }
                        ...
                        discordRpcClient.SetPresence(new RichPresence
                        {
                            ...
                            State = status.IsOnGround
                                ? "On the ground eating Nasgor ⌛✈️🧑‍✈️"
                                : $"✈ Flying at {Math.Round(status.Altitude)}ft {Math.Round(status.IndicatedAirSpeed)}kts On MSFS2020",

                            Assets = new Assets
                            {
                                LargeImageKey = "icon_large",
                                LargeImageText = tooltip.Trim(),
                                SmallImageKey = "dogepilot",
                                SmallImageText = "DogePilot, W plugin by soupfhaii_"
                            },
        ...
        private void SetPreparing()
        {
            try
            {
                ...
                discordRpcClient.SetPresence(new RichPresence()
                {
                    State = "✈️⛅ Doing Pre-flights on MSFS2020",
                    Assets = new Assets
                    {
                        LargeImageKey = "icon_large",
                        LargeImageText = "Microsoft Flight Simulator 2020",
                        SmallImageKey = "dogepilot",
                        SmallImageText = "DogePilot, W plugin by soupfhaii_"
                    }
                });
